...
object Day09 extends IOApp.Simple {
  ...
  def update(acc: Validator, number: Long): (Validator, Long) = {
    // Check possibilities
    val options = for {
  ...
  def parse(line: String) = line.toLong
  def collect(content: Stream[IO, Long]): Stream[IO, Validator] =
    content.mapAccumulate(Validator())(update).map(_._1)

  def process(stream: Stream[IO, Validator]): IO[Long] =
