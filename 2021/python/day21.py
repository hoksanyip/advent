...
# Import
#################################################
with open("2021/data/day21.txt", "r") as f:
...
roll_mat = np.array([np.roll(row, i + 1)
                    for i in range(n)], dtype=np.ulonglong)
roll_mat = roll_mat + roll_mat.dot(roll_mat) + \
    roll_mat.dot(roll_mat).dot(roll_mat)
