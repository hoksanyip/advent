...
# Import
#################################################
with open("2021/data/day08.txt", "r") as f:
...
class Digit:
    def __init__(self, parser: str):
        ...
        d2 = [d for d in digits if len(
            d) == 5 and not d.issubset(set.union(d4, d5))][0]
        d6 = [d for d in digits if len(d) == 6 and not d1.issubset(d)][0]
        d9 = [d for d in digits if len(
            d) == 6 and set.union(d3, d4).issubset(d)][0]
        d0 = [d for d in digits if len(
            d) == 6 and not (d4 - d1).issubset(d)][0]
