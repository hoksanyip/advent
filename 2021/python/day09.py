...
# Import
#################################################
with open("2021/data/day09.txt", "r") as f:
...
rel_diff[:, :-1] += (np.diff(-data, 1, 1) >
                     0).astype(int)  # higher than right?
...
def extend_basin(x: int, y: int, prev_value: int, data: np.array) -> set:
    ...
    if cur_value >= 9 or cur_value <= prev_value:
        ...
        acc = {(x, y)}
        acc = set.union(acc, extend_basin(
            x + 1, y, cur_value, data))  # go down
        acc = set.union(acc, extend_basin(x - 1, y, cur_value, data))  # go up
        acc = set.union(acc, extend_basin(
            x, y + 1, cur_value, data))  # go right
        acc = set.union(acc, extend_basin(
            x, y - 1, cur_value, data))  # got left
        return acc


basins = [len(extend_basin(point[0], point[1], -1, data_bounded))
